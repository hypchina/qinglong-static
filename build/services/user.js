...
let UserService = class UserService {
    ...
    async login(payloads, req, needTwoFactor = true) {
        ...
        if (content) {
            let { token: _token2, username: cUsername, password: cPassword, retries = 0, lastlogon, lastip, lastaddr, twoFactorActivated, twoFactorActived, tokens = {}, platform, } = content;
            // patch old field
            ...
            if (username === cUsername && password === cPassword) {
                // 登录成功，
                try {
                    // 尝试解码 JWT，如果过期，则会抛出错误
                    jsonwebtoken_1.default.verify(_token2, config_1.default.secret); // 'secret' 是签名的密钥
                    ...
                    return {
                        code: 200,
                        data: { token: _token2, lastip, lastaddr, lastlogon, retries, platform },
                    };
                }
                catch (error) {
                    // 如果错误是 TokenExpiredError，则 JWT 已过期
